...
if [[ "$action" == "all" ]]; then
    ...
    if [ -z "$buffer_clipboard_history" ]; then
        all_history=$(printf "%s" "${system_clipboard_history}" | eval "$CURRENT_DIR/.fzf-tmux $TMUX_FZF_OPTIONS")
    else
        all_history=$(printf "%s\n%s" "${buffer_clipboard_history}" "${system_clipboard_history}" | eval "$CURRENT_DIR/.fzf-tmux $TMUX_FZF_OPTIONS")
    fi
    [[ -z "${all_history}" ]] && exit
